...
## Implemented on 2026-04-03: Maxwell driver branch pin

- `modules/tesla-inference.nix` now treats `M40` and `M60` as a special driver
  case
  instead
  of
  following
  ...
  adds
  that
  attr, the module accepts
`config.boot.kernelPackages.nvidiaPackages.production` only when its version
still starts with `580.`.
- If Nixpkgs moves Maxwell off the 580 line without exposing a dedicated
`legacy_580` attr, evaluation now fails early with a clear message instead of
silently selecting a likely incompatible driver branch.

This keeps Maxwell support explicit and reviewable as NVIDIA moves older GPUs
onto maintained legacy branches.
